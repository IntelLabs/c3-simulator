...
#define LIM_MAX_LA_WIDTH 56 //< an object may not cross the user/supervisor boundary, so the max LA width is one less than the machine's supported LA width
#define LIM_MIN_PWR_SZ 5 //< minimum supported power size
...
/// of power sizes that need to be encoded, so we may as well directly encode the
/// the power size into the pointer.
/// Note that the minimumum is non-zero, which preserves the all-0 power size field value to
/// indicate an unencoded pointer when the pointer overall is canonical.
...
static inline void check_encoded_size (uint8_t encoded_size) {
    if (encoded_size < LIM_MIN_ENCODED_SZ || LIM_MAX_ENCODED_SZ < encoded_size) {
...
static inline int
is_encoded_address(uint64_t la)
{
    encoded_pointer_t* ep = (encoded_pointer_t*) &la;
    return (ep->enc_size !=0 && ep->enc_size != LIM_FMASK(LIM_SIZE_ENC_BITS)) ? 1 : 0;
}
...
static inline uint64_t lim_encode_pointer (uint64_t addr, uint8_t encoded_size, lim_tag_t tag) {
...
/// Expects an inclusive byte-granular bound as input expressed as a distance beyond
/// the first byte on the respective side of the midpoint.
///
...
/// Return the number of data bytes (i.e. excluding metadata storage) within the encoded
/// bounds, which are aligned to 8-byte boundaries.  Thus, the return value may exceed
/// the input data_size.
__attribute__((unused))
static size_t lim_compute_bounded_data_size (
        void* ptr, size_t data_size, size_t meta_size,
        uint64_t *out_enc_lb, uint64_t *out_enc_ub) {
    uint8_t encoded_size = calculate_encoded_size((uint64_t) ptr, data_size + meta_size);
    size_t slot_size = get_slot_size_in_bytes(encoded_size);
    ...
    // If the allocation does not reach the slot midpoint, this lower bound computation
    // will effectively "stretch" it to that point.  In other words, overflows prior to
    // the midpoint will not be detected in that case.  However, the next assertion will
    // reject any allocations that could have been fit to a tighter slot, so they will
    // not be allowed to be stretched in this way.  It is only allowed for allocations
    // in the smallest slot size.
    uint64_t lower_bound_in_bytes = (ptr_midpoint - 1) - (uint64_t) ptr;
    // inclusive upper bound that will be rounded up to 8B boundary:
    // The bounds encoding is such that the upper bound needs to extend at least 8B from the
    // midpoint.  Thus, even if an allocation does not extend beyond its metadata, the upper
    // bound will still include the 8B just beyond the midpoint.  This may effectively
    // stretch allocations' upper bounds.  However, note that allocations with 16B
    // metadata will never be stretched in this way, because the 8B of metadata itself above
    // the midpoint satisfies the minimum upper bound requirement.
    ...
    if (ptr_midpoint <= upper_la) {
        ...
        // There is always at least one byte of metadata beyond the midpoint, but encode_bound
        // rounds an inclusive byte bound of 0 up to 8, so leaving upper_bound_in_bytes set to
        // zero is acceptable.
    }
    // fprintf(stderr, ">>>>>> \n");
    ...
    // fprintf(stderr, "%s: ptr_metadata         = %p\n", __func__, ptr_metadata);
    // fprintf(stderr, "%s: lower_bound_in_bytes = %lx\n", __func__, lower_bound_in_bytes);
    // fprintf(stderr, "%s: upper_bound_in_bytes = %lx\n", __func__, upper_bound_in_bytes);
 
       
    uint64_t enc_lb = encode_bound(lower_bound_in_bytes);
    uint64_t enc_ub = encode_bound(upper_bound_in_bytes);

    if (out_enc_lb) *out_enc_lb = enc_lb;
    if (out_enc_ub) *out_enc_ub = enc_ub;
...
/// the input data_size.
__attribute__((unused))
static size_t set_metadata (void* ptr, size_t data_size, size_t meta_size, lim_tag_t tag) {
    uint8_t encoded_size = calculate_encoded_size((uint64_t) ptr, data_size + meta_size);
    void* ptr_metadata = (void*) get_metadata_address((uint64_t) ptr, encoded_size);

    uint64_t enc_lb, enc_ub;
    uint64_t bounded_data_sz = lim_compute_bounded_data_size(ptr, data_size, meta_size, &enc_lb, &enc_ub);
    if (meta_size == LIM_METADATA_SIZE_32_64B) {
        ...
        meta.tag            = tag;
#ifdef LIM_NO_ENCODE
        // When simply modeling allocator overheads, do not actually change the metadata, because this might be invoked from
        // realloc and corrupt data:
...
__attribute__((unused))
static lim_decoded_metadata_t lim_decode_metadata(void *metadata, size_t meta_size, uintptr_t la_middle) {
